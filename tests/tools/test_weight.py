...
def test_get_weight_summary_maps_fields(mock_weight_data_get, mock_login, fake_weight_data):
    ...
    assert result.weight_kg == pytest.approx(96.309)
    assert result.weight_delta_kg == pytest.approx(0.9)
    assert result.bmi == pytest.approx(28.799999237060547)
    assert result.body_fat_percent == 29.0
    assert result.body_water_percent == 51.8
    assert result.bone_mass_kg == pytest.approx(4.94)
    assert result.muscle_mass_kg == pytest.approx(35.68)
...
def test_get_weight_summary_missing_fields_fall_back_instead_of_crashing(
    ...
    mock_weight_data_get.return_value = _MinimalWeightData(weight=96309)
